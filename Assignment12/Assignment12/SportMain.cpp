...
// Orders sports by next scheduled game
void orderSports(int numSports, Sport* s)
{
	for (int i = 0; i < numSports - 1; i++)
	{
		for (int j = i + 1; j < numSports; j++)
		{
			if (s[i].getDate().year > s[j].getDate().year)
			{
				Sport temp = s[i];
				s[i] = s[j];
				s[j] = temp;
			}
			else if (s[i].getDate().year == s[j].getDate().year && s[i].getDate().month > s[j].getDate().month)
			{
				Sport temp = s[i];
				s[i] = s[j];
				s[j] = temp;
			}
			else if (s[i].getDate().year == s[j].getDate().year && s[i].getDate().month == s[j].getDate().month && s[i].getDate().day > s[j].getDate().day)
			{
				Sport temp = s[i];
				s[i] = s[j];
				s[j] = temp;
